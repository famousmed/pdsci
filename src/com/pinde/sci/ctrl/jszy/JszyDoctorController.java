...
import com.pinde.sci.biz.jszy.JszyDoctorRecruitBiz;
import com.pinde.sci.biz.jszy.JszyDoctorSingupBiz;
...
import com.pinde.sci.enums.jszy.JszyDocTypeEnum;
...
import com.pinde.sci.form.jszy.WorkResumeForm;
...
public class JszyDoctorController extends GeneralController{
	@Autowired
	...
	@Autowired
	private JszyDoctorSingupBiz doctorSingupBiz;
	@Autowired
	private JszyDoctorRecruitBiz doctorRecruitBiz;
	@Autowired
	...
	public String singup(Model model){
		...
		//此时只能查看报名信息
			...
			return "jszy/doctor/singupinfofordoctor";
		}
		...
		if(regDateCfgMsg.getAllowReg()){
			return "jszy/doctor/singup";	
		}else{
			model.addAttribute("regCfgMsg" ,regDateCfgMsg);
            return "jszy/doctor/notallowsingup";			
		}
	...
	public String swap(Model model){
		...
		for(ResDoctorRecruit rdr:recruits){
			if(GlobalConstant.FLAG_Y.equals(rdr.getRecruitFlag())){//目前是这个限制 ， 如果需要判断是否确认录取 是否强制学生点击拒绝录取
				...
				return "jszy/doctor/notallowswap";
			}
		}
		ResRecruitCfg recruitCfg = recruitCfgBiz.findRecruitCfgByYear(regYear);
		String currDate = DateUtil.getCurrDate();
	...
	public String submitSingup(SingUpForm form){
		...
		if (StringUtil.isNotBlank(doctor.getDoctorTypeId())) {
			doctor.setDoctorTypeName(JszyDocTypeEnum.getNameById(doctor.getDoctorTypeId()));
		}
	...
	public void print(String userFlow, HttpServletRequest request, HttpServletResponse response)throws Exception{
		...
		String path = "/jsp/jszy/print/printTemeplete.docx";//模板
...
//		return null;
	...
	public void printAdmissionCard(String userFlow, HttpServletRequest request, HttpServletResponse response)throws Exception{
		...
		WordprocessingMLPackage temeplete = new WordprocessingMLPackage();
		String path = "/jsp/jszy/print/admissionCardTemeplete.docx";//准考证模板
		ServletContext context =  request.getServletContext();
