...
function showGradeLine(){
	jboxPostLoad("content", "<s:url value='/hbres/grade/gradeline'/>", $("#examResultForm").serialize(), true);
}
function getPasscount(borderlineFlow , examFlow , speId , obj){
	...
	if(grade){
		jboxPostLoad("content", "<s:url value='/hbres/grade/getpasscount'/>", {"borderlineFlow":borderlineFlow,"examFlow":examFlow , "speId":speId , "gradeBorderline":grade}, true);
	}
}
...
function publishGradeBorderline(borderlineFlow , speId){
	jboxConfirm("确认发布？", function(){
		...
		jboxPostLoad("content", "<s:url value='/hbres/grade/publishgradeborderline'/>", {"borderlineFlow":borderlineFlow,"gradeStep":step}, true);
	});
