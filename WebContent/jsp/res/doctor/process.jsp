...
<script>
	...
	function loadRegistryView(){
		...
		jboxLoad("processDiv",url+"?roleFlag=${param.roleFlag}&schDeptFlow="+indexObj.schDeptFlow+"&rotationFlow="+indexObj.rotationFlow+"&recTypeId="+indexObj.recTypeId+"&resultFlow="+indexObj.resultFlow,true);
	}
	
	//登记
	function loadForm(recFlow,reqFlow,recTypeId){
		recTypeId = recTypeId || indexObj.recTypeId;
		jboxOpen("<s:url value='/res/rec/showRegistryForm'/>?roleFlag=${param.roleFlag}&reqFlow="+reqFlow+"&recFlow="+recFlow+"&schDeptFlow="+indexObj.schDeptFlow+"&rotationFlow="+indexObj.rotationFlow+"&recTypeId="+recTypeId+"&resultFlow="+indexObj.resultFlow,title[recTypeId].registry,title[recTypeId].width,title[recTypeId].height);
	}
	
	//申述
	function editAppeal(appealFlow,reqFlow,recTypeId){
		recTypeId = recTypeId || indexObj.recTypeId;
		jboxOpen("<s:url value='/res/rec/editAppeal'/>?roleFlag=${param.roleFlag}&schDeptFlow="+indexObj.schDeptFlow+"&rotationFlow="+indexObj.rotationFlow+"&recTypeId="+recTypeId+"&reqFlow="+reqFlow+"&appealFlow="+appealFlow+"&resultFlow="+indexObj.resultFlow,title[indexObj.recTypeId].appeal,700,400);
	}
...
<!-- 			</div> -->
			...
			<div class="side" style="height: 100%;width: 270px;">
				<c:forEach items="${arrResultList}" var="result">
					<div resultFlow="${result.resultFlow}" processFlow="${processMap[result.resultFlow].processFlow}" class="box ${param.resultFlow eq result.resultFlow?'selected':''}" 
