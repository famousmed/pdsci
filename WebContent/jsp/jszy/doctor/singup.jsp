...
function uploadFile(type,typeName) {
	jboxOpen("<s:url value='/inx/jszy/uploadFile'/>?operType="+type,"上传"+typeName,450,150);
}
...
function getRecruit(){
	jboxLoad("content" , "<s:url value='/jszy/doctor/getrecruit'/>" , true);
...
function submitSingup(){
	...
	jboxPost("<s:url value='/jszy/doctor/submitSingup'/>" , $("#doctorForm").serialize() , function(resp){
		if(resp=="1"){
			getRecruit();
		}else{
			jboxTip(resp);
