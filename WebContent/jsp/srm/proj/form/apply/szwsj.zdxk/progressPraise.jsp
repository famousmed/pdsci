...
function addProj(tb,obj){
	...
	'<td><input type="text" class="inputText"  name="praise_praiseTime"   onclick="WdatePicker({dateFmt:'+"'yyyy-MM-dd'"+'})" style="width:80%;"/></td>'+
	...
	$("#"+tb+"Tb").children("tr").last().children("td").eq(1).text(length+1);
        	    	...
        	    	<form action="<s:url value='/srm/proj/mine/saveStep'/>" method="post" id="projForm" style="position: relative;">
						<input type="hidden" name="pageName" value="progressPraise"/>
						<input type="hidden" id="recFlow" name="recFlow" value="${projRec.recFlow}"/>
                        <input type="hidden" id="projFlow" name="projFlow" value="${proj.projFlow}"/>
						<input type="hidden" name="recTypeId" value="${param.recTypeId}"/>
   					<table class="bs_tb" style="width: 100%;margin-top: 10px;" id="praise">
   						<tr>
	   						<th colspan="7" class="theader">七、近五年为第一完成人的市级及以上科技进步奖共 <input type="text" name="praiseNum" value="${resultMap.praiseNum}" class="validate[custom[integer]] inputText" style="width:30px;"/> 项。（限填10项代表作）
	   						<c:if test="${param.view!=GlobalConstant.FLAG_Y }">
          			...
          			<c:if test="${empty resultMap.praise}">
          			   <tr>
				           ...
				           <td>
				           		<input type="text"  class="inputText"  name="praise_praiseTime" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:80%;"/>
				           </td>
				           <td>
					...
					<c:forEach var="praise" items="${resultMap.praise}" varStatus="status">
				        <tr>
				           ...
				           <td>
				           		<input type="text"  class="inputText"  name="praise_praiseTime" value="${praise.objMap.praise_praiseTime}"     onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width:80%;"/>
				           </td>
				           <td>
                	 ...
                	 <c:if test="${param.view == GlobalConstant.FLAG_Y}">display:none;</c:if>">
                	 	<input id="prev" type="button" onclick="nextOpt('researchProj')" class="search" value="上一步"/>
                	    <input id="nxt" type="button" onclick="nextOpt('SCIAndThesis')" class="search" value="下一步"/>
      			     </div>
