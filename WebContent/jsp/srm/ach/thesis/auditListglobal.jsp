...
function edit(thesisFlow){
	jboxStartLoading();
	jboxOpen("<s:url value='/srm/ach/thesis/edit?thesisFlow='/>"+ thesisFlow+"&editFlag=${GlobalConstant.FLAG_N}", "查看专利信息",width, height);
}
		  ...
		  <form id="searchForm" action="<s:url value="/srm/ach/thesis/auditList/global"/>" method="post">
			 <p>
				...
				<select name="projSourceId" class="xlselect">
					...
					<option value="${dict.dictId }" <c:if test="${param.projSourceId eq dict.dictId }">selected="selected"</c:if>>${dict.dictName }</option>
					</c:forEach>
				</select>
			</p>
			<p>
