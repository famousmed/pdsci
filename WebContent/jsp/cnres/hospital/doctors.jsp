...
<script type="text/javascript">
	...
	function noticeRetestMain(recruitFlow){
		jboxOpen("<s:url value='/cnres/singup/hospital/noticeRetestMain'/>?operType=single&recruitFlow="+recruitFlow,"复试通知",1000,500);
	}
	...
	function gradeEdit(recruitFlow){
		jboxOpen("<s:url value='/cnres/singup/hospital/gradeedit'/>?recruitFlow="+recruitFlow , "成绩录入" , 400 , 200);
	}
	
	//是否录取操作
	function admitOper(recruitFlow , admitFlag){
		if(admitFlag=='${GlobalConstant.FLAG_Y}'){
			jboxOpen("<s:url value='/cnres/singup/hospital/noticeRecruitMain'/>?operType=single&recruitFlow="+recruitFlow,"录取通知",1000,500);
		}else if(admitFlag=='${GlobalConstant.FLAG_N}'){
			jboxConfirm("确认不录取？", function(){
				jboxPost("<s:url value='/cnres/singup/hospital/notrecruit'/>" , {"recruitFlow":recruitFlow} , function(resp){
	...
	function noticeRecruitMainForBatch(){
		...
		jboxOpen("<s:url value='/cnres/singup/hospital/noticeRecruitMain'/>?operType=batch","录取通知",1000,500);
	}
	
	//调剂操作
	function swapOper(recruitFlow , speId){
		jboxOpen("<s:url value='/cnres/singup/hospital/noticeSwapMain'/>?recruitFlow="+recruitFlow+"&speId="+speId,"调剂专业",1000,500);
	}
