...
function saveExam(){
	...
	jboxConfirm("确认提交？", function(){
		var url = "<s:url value='/cnres/singup/exam/save'/>";
		var loadPage = "add";
		if($('#examFlow').val()){
			loadPage = "edit";
		}
...
function editExam(examFlow){
	var url = "<s:url value='/cnres/singup/exam/findexam'/>?examFlow="+examFlow;
...
function finishExam(examFlow){
	jboxConfirm("确认考试安排结束？结束后不可再对该场考试操作！",function(){
		jboxPost("<s:url value='/cnres/singup/exam/finishexam'/>",{"examFlow":examFlow},function(resp){
			if(resp=="1"){
				jboxTip("操作成功");
				examPlan();
			}else{
...
function intoExam(examFlow){
	jboxLoad("content","<s:url value='/cnres/singup/exam/intoexam'/>?examFlow="+examFlow,true);
}
</script>
      <div class="main_hd">
        <h2>考试管理</h2>
