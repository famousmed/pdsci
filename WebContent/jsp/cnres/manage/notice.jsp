...
function saveNotice(){
	if($("#editForm").validationEngine("validate")){
		...
		var url = "<s:url value='/cnres/notice/save'/>";
...
function edit(infoFlow){
	var url = "<s:url value='/cnres/notice/findnoticebyflow'/>?infoFlow="+infoFlow;
...
function delNotice(infoFlow){
	jboxConfirm("确认删除？" , function(){
		var url = "<s:url value='/cnres/notice/delnotice'/>?infoFlow="+infoFlow;
		jboxGet(url , null , function(resp){
			notice();
		} , null , true);
	});
...
<h3>公告编辑
    ...
    <a title="收缩" id="up" onclick="editFormHidden(true);">
        <img src="<s:url value='/jsp/cnres/images/up.png'/>"/>
    </a>
    <a title="展开" id="down" onclick="editFormHidden(false);" style="display: none">
        <img src="<s:url value='/jsp/cnres/images/down.png'/>"/></a>
    </span>
</h3>
            ...
            <li class="docDiv">
                <a>
					...
					<span class="editSpan" style="display:none;">
						...
						<a href="<s:url value='/cnres/notice/view'/>?infoFlow=${msg.infoFlow}" target="_blank" class="bl">查看</a> |
