...
$(document).ready(function(){
    ...
    function initBindSpeInfo(combobox , speId){
    	combobox.bind("change" , function(){
    		...
    		var url = "<s:url value='/cnres/singup/doctor/findspe'/>?orgFlow="+orgFlow;
	...
	function confirmRecruit(recruitFlow , confirmFlag){
		...
		jboxConfirm(tip , function(){
			jboxPost("<s:url value='/cnres/singup/doctor/confirmrecruit'/>" , {"recruitFlow":recruitFlow,"confirmFlag":confirmFlag} , function(resp){
				if(resp=="1"){
					jboxTip("操作成功");
	...
	function delRecruit(recruitFlow){
		jboxConfirm("确认撤销？", function(){
			jboxGet("<s:url value='/cnres/singup/doctor/delrecruit'/>?recruitFlow="+recruitFlow , null , function(resp){
				if(resp=="1"){
					top.jboxTip("操作成功");
					location.href="<s:url value='/cnres/singup/doctor/scoreSearch'/>";
				}else{
	...
	function showJidiRecruitInfo(){
		jboxOpen("<s:url value='/cnres/singup/doctor/showjidirecruitinfo'/>","基地招录信息",1000,500);
	}
...
<body>
  <div class="main_wrap">
    ...
    <form id="recruitForm" name="recruitForm" method="post" action="<s:url value='/cnres/singup/doctor/submitrecruit'/>">
    <input type="hidden" name="examResult" value="${examDoctor.examResult}"></input>
    <div class="zy_contain bs_tbzy">
      <h1>填报志愿 
          <span style="color: green;font-size: 18px;" id="msg">${param.msg}</span>
   ...
   <c:if test="${doctorRecruit.retestFlag eq 'Y'}">
    ...
    <div class="zy_contain bs_fscj">
      ...
      <ul>
        ...
        <li class="bs_cj"><!-- <img src="<s:url value='/jsp/cnres/images/basic/bhg_small.png'/>" /> --></li>
