		       		 ...
		       		 <c:forEach items="${projDescForm.visitElementAttributeMap[commAttrCode] }" var="attr" varStatus="status1">
		       		 	{
		       		 		...
		       		 		<c:choose>
		       		 			<c:when test="${oper=='oper1' }">
		       		 				...
		       		 				<c:set var="inputValue" value="${elementSerialSeqValueMap[element.elementCode]['1'][attr.attrCode].attrValue1}"/>
		       		 			</c:when>
		       		 			<c:when test="${oper=='oper2' }">
		       		 				"inputValue":"${elementSerialSeqValueMap[element.elementCode]['1'][attr.attrCode].attrValue2}",
		       		 				<c:set var="inputValue" value="${elementSerialSeqValueMap[element.elementCode]['1'][attr.attrCode].attrValue2}"/>
		       		 			</c:when>
		       		 			<c:when test="${oper=='oper3' }">
		       		 				"inputValue":"${elementSerialSeqValueMap[element.elementCode]['1'][attr.attrCode].attrValue}",
		       		 				<c:set var="inputValue" value="${elementSerialSeqValueMap[element.elementCode]['1'][attr.attrCode].attrValue}"/>
		       		 			</c:when>
		       		 			<c:otherwise>
		       		 				"inputValue":"",
		       		 			</c:otherwise>
